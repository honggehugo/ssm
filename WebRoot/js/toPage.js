  ...
  function page_even() {
    ...
    function change_content() {
      ...
      for(var i=0;i<per_num;i++){
        page_content+="<div class='companyItem'><a href='<%=request.getContextPath() %>/queryQyById.do?cpyId=${list.cpyId}'><img src='img/defaultCompany.jpg'><div class='companyBoxR'><span class='s-1'>${list.cpyName}</span><span class='s-2'>${list.other3}</span><span>${list.other2}</span><c:if test='${list.cpyDesc.length() <= 100}'><p>${list.cpyDesc}</p></c:if><c:if test='${list.cpyDesc.length() > 100}'><p>${list.cpyDesc.substring(0,100)}......</p></c:if></div></a></br></div>";
      }
